...
import orjson
import pytest
from infra.message_broker.brokers import MessageBrokerImpl
from infra.message_broker.main import build_channel_pool, build_connnection_pool
...
class TestMessageBrokerImpl:
    ...
    async def test_publish_message(self, channel: aio_pika.abc.AbstractChannel) -> None:
        ...
        exchange = await message_broker.declare_exchange("tests")
        queue: aio_pika.abc.AbstractQueue = await channel.declare_queue("tests")
        queue = await channel.declare_queue("tests")
        await queue.bind(exchange, routing_key="tests")

        await message_broker.publish_message(message=reference_message, routing_key="", exchange_name="tests")
